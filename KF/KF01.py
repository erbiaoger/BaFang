...
class TrackingGUI(QMainWindow):
    ...
    def redraw_plot(self):
        ...
        self.tracking.tracking_visualization_one_section(
            ...
            weak_states=self.veh_weak_states,
            fig=self.fig,
            ax=self.ax
        )
        # 如果有选中的车辆，用粗线高亮
        if self.selected_vehicle is not None:
            idx = self.selected_vehicle
            # 提取该车所有有效点
            cols = np.where(~np.isnan(self.veh_states[idx]))[0]
            ts = (self.tracking.t_axis[self.veh_states[idx, cols].astype(int)])
            xs = (self.tracking.x_axis[cols] * 1e-3)
            self.ax.plot(ts, xs, '-', linewidth=3, label=f"选中车辆 {idx}", color='orange')
            self.ax.legend()
        self.canvas.draw()
    ...
    def on_click(self, event):
        ...
        clicked_x = event.ydata * 1e3       # km→m
        # 对应的索引
        ...
        # 右键：选车
        if event.button == 3:
            # 在该横向位置 x_idx 上，找出所有有观测的车辆
            candidates = [
                v for v in range(self.veh_states.shape[0])
                if not np.isnan(self.veh_states[v, x_idx])
            ]
            if not candidates:
                print("⚠️ 此位置没有任何车辆轨迹可选。")
                self.selected_vehicle = None
            else:
                # 选最接近点击时间的那条
                times = [
                    self.tracking.t_axis[int(self.veh_states[v, x_idx])]
                    for v in candidates
                ]
                diffs = [abs(tt - clicked_t) for tt in times]
                self.selected_vehicle = candidates[int(np.argmin(diffs))]
                print(f"✅ 选中车辆编号：{self.selected_vehicle}")
            self.redraw_plot()

        # 左键：补追踪
        elif event.button == 1:
            if self.selected_vehicle is None:
                print("❗️ 请先右键选择要补追踪的车辆。")
                return
            ...
            # new_states 形状 (1, N)，取 [0] 拆成一维
            track_len = new_states.shape[1]
            # 更新选中车辆，从 x_idx 列开始替换
            self.veh_states[self.selected_vehicle, x_idx:x_idx+track_len] = new_states[0]
            self.veh_weak_states[self.selected_vehicle, x_idx:x_idx+track_len] = new_weak[0]

            print(f"✅ 车辆 {self.selected_vehicle} 补追踪完成（从列 {x_idx} 开始，共 {track_len} 点）。")
            self.redraw_plot()
...
if __name__ == "__main__":
    ...
    cfts   = np.load('/Users/zhangzhiyu/MyProjects/BaFang/vehicle_track/save_data/agc_20250728_153911.npz')['data']
